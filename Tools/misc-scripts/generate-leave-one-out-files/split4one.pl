...
# getting a file (first parameter) assume is csv file WITH  header (!); fields are information but the second si  0/1 define if positiive or negative example
# create a set of training/test pairs to trick CNN eval ...
#
# second param is label used to distinguish projects/scripts; for example run:
#
# ./split4one.pl OracleV5C14.csv lapd
#
# it will create a directory OracleV5C14-training and it assume such a directory will be
# copied under the directory data (see https://github.com/dennybritz/cnn-text-classification-tf)
# thus the list of training pairs (file train.txt) contain lines such as:
#
# data/OracleV5C14-training/43-train-lapd-no.neg:data/OracleV5C14-training/43-train-lapd-yes.pos
#
...
shift @lines; # remove first line the header
