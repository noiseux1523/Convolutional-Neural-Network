...
# Load data bvia the positive/negatives test data path (command parameter)
...
if FLAGS.eval_train and datasets is None:
    if dataset_name == "mrpolarity" or dataset_name == "argoUML":
        datasets = data_helpers.get_datasets_mrpolarity(cfg["datasets"][dataset_name]["test_positive_data_file"]["path"],
...
if y_test is not None:
    ...
    matrix = metrics.confusion_matrix(y_test, all_predictions)
    recall      = float(matrix[0,0])/(float(matrix[0,0])+float(matrix[0,1]))
    precision   = float(matrix[0,0])/(float(matrix[0,0])+float(matrix[1,0]))
    specificity = float(matrix[1,1])/(float(matrix[1,1])+float(matrix[1,0]))
    F1          = 2*float(matrix[0,0])/(2*float(matrix[0,0])+float(matrix[1,0])+float(matrix[0,1]))
    print("Recall: {}, Precision: {}, Specificity: {}, F1: {}".format(recall, precision, specificity, F1))
