...
cat $TaskName | while read train; 
	...
	./train.py --where runs/$FolderName/$NumberFilters-$FilterSize/ --enable_word_embeddings true --num_epochs 100 --num_filters $NumberFilters --filter_sizes "$FilterSize"  --dev_sample_percentage 0.1 --positive_train=$pos --negative_train=$neg --ymlconfig configC14.yml # added giulio to stop after few iteration and check if needed ..
