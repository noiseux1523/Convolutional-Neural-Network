...
find ./runs/$FolderName  -maxdepth 6 -name "checkpoints" |sort -t'/' -k5 -n | while read dir; # making sure we have the right order
	...
	./eval_LOO.py --eval_train --checkpoint_dir=$dir --positive_test=$pos --negative_test=$neg --ymlconfig data/$FolderName/ConfigTraining.yml > $dir/.stats;  
...
echo "TestFile, TP, FN, FP, TN" > ./runs-good/$FolderName/rough-stats.csv
