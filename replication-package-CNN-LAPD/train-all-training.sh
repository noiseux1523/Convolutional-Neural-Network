...
cat $TaskName | while read train; 
	...
	./train.py --where runs/$FolderName/$it/ --enable_word_embeddings true --num_epochs 20 --dev_sample_percentage 0.1 --positive_train=$pos --negative_train=$neg --ymlconfig data/$FolderName/ConfigTraining.yml # added giulio to stop after few iteration and check if needed ..
